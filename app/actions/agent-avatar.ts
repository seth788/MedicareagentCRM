...
export async function uploadAgentAvatar(
  ...
  try {
    ...
    // Use service role for upload - bypasses storage RLS (auth validated above)
    let storageClient
    try {
    ...
    await profilesDb.updateProfileAvatar(user.id, publicUrl)

    return { imageUrl: publicUrl }
  } catch (e) {
    console.error("uploadAgentAvatar", e)
